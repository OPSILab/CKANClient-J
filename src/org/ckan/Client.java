...
public final class Client {
    ...
    */
    public Dataset getDataset(String name) throws CKANException, MalformedURLException, SocketTimeoutException {
...
//        		throw new ODMSNodeNotFoundException(" The ODMS host does not exist");
        		throw new CKANException("The ODMS host does not exist");
        	}else if(returned_json.contains("403")){
//        		throw new ODMSNodeForbiddenException(" The ODMS node is forbidden");
        		throw new CKANException(" The ODMS node is forbidden");
        	}else{
//        		throw new ODMSNodeOfflineException(" The ODMS node is currently unreachable");
    ...
    * @throws A CKANException if the request fails
    */
    public void deleteDataset(String name) throws CKANException, MalformedURLException, SocketTimeoutException {
 ...
 * @throws SocketTimeoutException 
    ...
    * @throws A CKANException if the request fails
    */
    public Dataset.SearchResults findDatasets(String query, String start, String rows,String sort) throws CKANException, MalformedURLException, SocketTimeoutException{
    	...
    	do{
    		...
    		if(returned_json.startsWith("{")){
    			...
    			if(query.contains("metadata_modified") && sort.equals("metadata_modified asc") && rows.equals("10000000")){
    				flag++;
    				logger.info("Exception attempt n: "+flag);
    				if(flag==5) stop=false;
    			}else{
    				stop=false;
    			}
    		}
        ...
        //int flag=0;
//        if(returned_json.equals("")){
//        	returned_json=this._connection.Post("/api/action/package_search",payload);
//        	logger.info("Returned JSON_0 "+returned_json);
//        	if(returned_json.equals("")){
//            	returned_json=this._connection.Post("/api/action/package_search",payload);
//            	logger.info("Returned JSON_1 "+returned_json);
//            	if(returned_json.equals("")){
//                	returned_json=this._connection.Post("/api/action/package_search",payload);
//                	logger.info("Returned JSON_2 "+returned_json);
//                }
//            }
//        }
//        logger.info("HERE");
...
//        logger.info("HERE1");
        if(!returned_json.startsWith("{")){
//        	logger.info("ERROR");
//        	logger.info(returned_json.toString());
        	if(returned_json.matches(".*The requested URL could not be retrieved.*")){
//        		throw new ODMSNodeNotFoundException(" The ODMS host does not exist");
        		throw new CKANException(" The ODMS host does not exist");
        	}
...
//        		throw new ODMSNodeOfflineException(" The ODMS node is currently unreachable");
     ...
     * @throws A CKANException if the request fails
     */
     public String[] getAllDatasetsID()  throws CKANException, MalformedURLException, SocketTimeoutException {
    	    	 
    	 String returned_json = this._connection.Post("/api/action/package_list","");
...
//         	throw new ODMSNodeOfflineException(" The ODMS node is currently unreachable");
     ...
     * @throws ODMSNodeForbiddenException 
      ...
      * @throws A CKANException if the request fails
      */
      public List<Dataset> getAllDatasets(String offset, String limit)  throws CKANException, MalformedURLException,SocketTimeoutException {
     	 logger.info("GET ALL DATASET CLIENT");
    	  String returned_json = this._connection.Post("/api/action/current_package_list_with_resources","{\"limit\":\"" + limit + "\",\"offset\":\"" + offset + "\"}");
    	  logger.info("GET ALL DATASET CLIENT ------- END");
//    	  logger.info(returned_json);
		 if(!returned_json.startsWith("{")){
			 if(returned_json.matches(".*The requested URL could not be retrieved.*")  || returned_json.matches(".*does not exist.*")){
//				 throw new ODMSNodeNotFoundException(" The ODMS host does not exist");
				 throw new CKANException(" The ODMS host does not exist");
			 }else if(returned_json.contains("403")){
//				 throw new ODMSNodeForbiddenException(" The ODMS node is forbidden");
...
//				 throw new ODMSNodeOfflineException(" The ODMS node is currently unreachable");
      ...
      *
      * Makes an Hashmap where every key-value pair is a corrispondence between Dataset ID and 
      * related activity on it, starting from a passed starting Date.
      * 
      * @param  name The string representing the starting date compliant to ISO 8601 standard
     ...
     * @throws SocketTimeoutException 
      ...
      * @throws A CKANException if the request fails
      *//*
     public HashMap<String,String> getChangedDatasetsID(String startingDateString)  throws CKANException, JSONException, ParseException, MalformedURLException, ODMSNodeOfflineException, SocketTimeoutException {
    	
    	 System.out.println(startingDateString);
    	 HashMap<String,String> changedDatasets = new HashMap<String, String>();
...
//    	 GregorianCalendar startingDate = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
    	 ...
    	 JSONArray changedArray;
    	 String packageID, packageActivity;
    	 
    	 
    	 
    	 int offset=0;
    	 boolean stop=false; // stop flag for the global request
    	 boolean retry;  // retry flag for each API request
    	 int retryNum = 0; // number of retry for each API request
    	 String returned_json;
    	 
    	 while(!stop){
    	 
	    	 retry = true;
	    	 // Try to perform the API request, for a max number set by retryNum
    		 do{
	    		 // Send the API request
		    	 returned_json = this._connection.Post("/api/action/recently_changed_packages_activity_list","{\"limit\":10000,\"offset\":"+offset+"}");
		    	 if(returned_json.startsWith("{"))
		       			retry=false;			
		    	 else{
		    			retryNum++;
		    			logger.info("Exception attempt n: "+ retryNum);
		    			if(retryNum==5) retry=false;
		    	} 
    		 }while(retry);
	    	 ...
	    	 for(int i=0;i<changedArray.length();i++){
	    		 JSONObject obj = changedArray.getJSONObject(i);
	    		 String timestampObj = obj.getJSONObject("data").getJSONObject("package").getString("metadata_modified");
	    		// String timestampObj = obj.getString("timestamp");//.getJSONObject("package").getString("metadata_modified");
	    		 JSONObject packageObj = obj.getJSONObject("data").getJSONObject("package");
	    		 GregorianCalendar objectDate = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
	    		 objectDate.setLenient(false);
	    		 objectDate.setTimeInMillis(sdf.parse(timestampObj.substring(0, timestampObj.length()-7)+"Z").getTime());
	    		
	    			packageID = packageObj.getString("id");
	    			packageActivity = obj.getString("activity_type");
	    	
	    		 
	    			if(objectDate.after(startingDate) && !packageActivity.contains("deleted")){
	    				 
	        			
	        			// Checks if changedDataset was created and then deleted from startingDate
	        			// If true this dataset is not to be added to (or removed from) changedDatasets.
	        			// In addition, because retrieved actions are ordered from newest to oldest,
	        			// if a dataset is marked as deleted, previous actions should not to be added
	        			// in changed datasets list.
	        			// In addition if a dataset is newly created, only this action is to be added
	        			// except for deletion
	        			
	        			if ( changedDatasets.containsKey(packageID)){
	        				if( changedDatasets.get(packageID).equals("deleted package") && packageActivity.equals("new package")){
	        					changedDatasets.remove(packageID);
	        					continue;
	        				}
	        				else if ( changedDatasets.get(packageID).equals("deleted package") && packageActivity.equals("changed package") )
	        					continue;
	        				else if ( changedDatasets.get(packageID).equals("changed package") && packageActivity.equals("new package") )
	        					continue;
	        				else if ( changedDatasets.get(packageID).equals("changed package") && packageActivity.equals("changed package") )
	        					continue;
	        			}
	        			changedDatasets.put(packageID,packageActivity);		 
	        		 }else if (packageActivity.contains("deleted")){
	        			 changedDatasets.put(packageID,packageActivity);
	        		 }else{
	        			 //I dataset sono ordinati per metadata_modified desc 
	        			 // quindi al primo dataset con objectDate.before(startingDate) possiamo fermarci
	        			 stop=true;
	        			 break;
	        		 }	    		 
	    	 }
